    ...
    public ArrayList pathOfNodes(l1nodes startNode, l1nodes endNode) {
        ...
        while (!currentNode.getNodeID().equals(endNode.getNodeID())) {
            ...
            for (int i = 0; i < currentNode.edgeCount(); i++) {
                ...
                if (currentEdge.getStartNode().getNodeID().equals(currentNode.getNodeID())) { //check whether node is starting node or ending node in the edge
                    otherNode = endNode;
                } else {
                    otherNode = startNode;
                }
                ...
                for (int j = 0; j < visited.size(); j++) {
                    if (otherNode.getNodeID().equals(visited.get(j).getNodeID())) { //check if node has been visited
                        otherVisited = true;
                    }
                }
